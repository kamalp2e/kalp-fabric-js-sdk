...
class Context {
    ...
    async getKYC(userId) {
		...
		console.log("invokechaincode response", response )
		byte[] bytes = encoding.GetBytes(userId);
		console.log("convertging bytes")
        let response = await this.stub.invokeChaincode(crossCCName, bytes, channelName)
		console.log("converted bytes")
	...
	async putStateWithKYC(key, value) {
		...
		console.log("----------putStateWithKYC and kyc *******-------------", userid)
		
		let useridbytes = this.getUserIdBytes() 
		console.log("----------putStateWithKYC and kyc *******-------------", useridbytes)
		
		// let userid ="Smartxx5"
		let res = await this.getKYC(userid)
