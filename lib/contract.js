...
class Kalpcontract {
    ...
    */
    ...
    async afterTransaction(ctx, result) {
        // default implementation is do nothing
        console.log("@@@@@@@@Is contract payable:", this.IsPayableContract);
        ...
        if  this.IsPayableContract {
            ...
            paymentData = paymentTracker.PaymentMetaData
            paymentData.DocType= "PAYMENT-INFO"
			paymentData.TransactionId = ctx.GetTxID()
            paymentData.AssetId = paymentTracker.Id
            paymentData.AssetDocType = paymentTracker.DocType
            console.log("paymentData", paymentData )
            await ctx.stub.putState(carNumber, Buffer.from(JSON.stringify(paymentData)));
        }
    }
