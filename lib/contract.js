...
class Kalpcontract {
    ...
    */
    constructor(name, IsPayableContract) {
        console.info('============= START : Kalpcontract constructor ===========');
        ...
        if (typeof IsPayableContract === 'undefined' || IsPayableContract === null) {
            console.info('============= isPayableContract null ===========');

            this.IsPayableContract = false;
        } else {
            console.info('============= isPayableContract true ===========');
    ...
    async afterTransaction(ctx, result) {
        // default implementation is do nothing
        logger.log("After Transaction:", ctx.stub.getTxID(), "IsPayable is:", this.IsPayableContract);
        const {fcn, parameters} = ctx.stub.getFunctionAndParameters()
        
        if  (this.IsPayableContract) {
            logger.log(" inside blodck IsPayableContract")
            let paymentTracker = JSON.parse(parameters) 
            let paymentData = paymentTracker.PaymentMetaData       
            if paymentData.amount < 1 || paymentData.CucurrencyCode == "" {
